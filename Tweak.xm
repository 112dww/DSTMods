...
int DST_CreateModsFolder(void)
{
  ...
  withIntermediateDirectories:YES
    ...
    NSString *enabled=[DSTRoot() stringByAppendingPathComponent:@"enabledmods.lua"];

    if(![fm fileExistsAtPath:enabled]){
        [@"return {}\n" writeToFile:enabled atomically:YES encoding:NSUTF8StringEncoding error:nil];
    }

    NSString *setting=[DSTRoot() stringByAppendingPathComponent:@"modsettings.lua"];

    if(![fm fileExistsAtPath:setting]){
        [@"return {}\n" writeToFile:setting atomically:YES encoding:NSUTF8StringEncoding error:nil];
    }
